# coding=utf-8
# *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import copy
import warnings
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
from . import _utilities

__all__ = [
    'LegacyGetPrivatelinkEndpointResult',
    'AwaitableLegacyGetPrivatelinkEndpointResult',
    'legacy_get_privatelink_endpoint',
    'legacy_get_privatelink_endpoint_output',
]

warnings.warn("""Do not use! This resource is mapped from the legacy Terraform `dbt_cloud_`-prefixed resource/datasource""", DeprecationWarning)

@pulumi.output_type
class LegacyGetPrivatelinkEndpointResult:
    """
    A collection of values returned by LegacyGetPrivatelinkEndpoint.
    """
    def __init__(__self__, cidr_range=None, id=None, name=None, private_link_endpoint_url=None, state=None, type=None):
        if cidr_range and not isinstance(cidr_range, str):
            raise TypeError("Expected argument 'cidr_range' to be a str")
        pulumi.set(__self__, "cidr_range", cidr_range)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if name and not isinstance(name, str):
            raise TypeError("Expected argument 'name' to be a str")
        pulumi.set(__self__, "name", name)
        if private_link_endpoint_url and not isinstance(private_link_endpoint_url, str):
            raise TypeError("Expected argument 'private_link_endpoint_url' to be a str")
        pulumi.set(__self__, "private_link_endpoint_url", private_link_endpoint_url)
        if state and not isinstance(state, int):
            raise TypeError("Expected argument 'state' to be a int")
        pulumi.set(__self__, "state", state)
        if type and not isinstance(type, str):
            raise TypeError("Expected argument 'type' to be a str")
        pulumi.set(__self__, "type", type)

    @property
    @pulumi.getter(name="cidrRange")
    def cidr_range(self) -> str:
        return pulumi.get(self, "cidr_range")

    @property
    @pulumi.getter
    def id(self) -> str:
        return pulumi.get(self, "id")

    @property
    @pulumi.getter
    def name(self) -> Optional[str]:
        return pulumi.get(self, "name")

    @property
    @pulumi.getter(name="privateLinkEndpointUrl")
    def private_link_endpoint_url(self) -> Optional[str]:
        return pulumi.get(self, "private_link_endpoint_url")

    @property
    @pulumi.getter
    def state(self) -> int:
        return pulumi.get(self, "state")

    @property
    @pulumi.getter
    def type(self) -> str:
        return pulumi.get(self, "type")


class AwaitableLegacyGetPrivatelinkEndpointResult(LegacyGetPrivatelinkEndpointResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return LegacyGetPrivatelinkEndpointResult(
            cidr_range=self.cidr_range,
            id=self.id,
            name=self.name,
            private_link_endpoint_url=self.private_link_endpoint_url,
            state=self.state,
            type=self.type)


def legacy_get_privatelink_endpoint(name: Optional[str] = None,
                                    private_link_endpoint_url: Optional[str] = None,
                                    opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableLegacyGetPrivatelinkEndpointResult:
    """
    Use this data source to access information about an existing resource.
    """
    pulumi.log.warn("""legacy_get_privatelink_endpoint is deprecated: Do not use! This resource is mapped from the legacy Terraform `dbt_cloud_`-prefixed resource/datasource""")
    __args__ = dict()
    __args__['name'] = name
    __args__['privateLinkEndpointUrl'] = private_link_endpoint_url
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('dbtcloud:index/legacyGetPrivatelinkEndpoint:LegacyGetPrivatelinkEndpoint', __args__, opts=opts, typ=LegacyGetPrivatelinkEndpointResult).value

    return AwaitableLegacyGetPrivatelinkEndpointResult(
        cidr_range=pulumi.get(__ret__, 'cidr_range'),
        id=pulumi.get(__ret__, 'id'),
        name=pulumi.get(__ret__, 'name'),
        private_link_endpoint_url=pulumi.get(__ret__, 'private_link_endpoint_url'),
        state=pulumi.get(__ret__, 'state'),
        type=pulumi.get(__ret__, 'type'))


@_utilities.lift_output_func(legacy_get_privatelink_endpoint)
def legacy_get_privatelink_endpoint_output(name: Optional[pulumi.Input[Optional[str]]] = None,
                                           private_link_endpoint_url: Optional[pulumi.Input[Optional[str]]] = None,
                                           opts: Optional[pulumi.InvokeOptions] = None) -> pulumi.Output[LegacyGetPrivatelinkEndpointResult]:
    """
    Use this data source to access information about an existing resource.
    """
    pulumi.log.warn("""legacy_get_privatelink_endpoint is deprecated: Do not use! This resource is mapped from the legacy Terraform `dbt_cloud_`-prefixed resource/datasource""")
    ...
